...
while [ "$current_date" -le "$end_date_seconds" ]; do
	...
	# Create a proper ini file for stop-and-go simulations: 1) read sno file from current_snow directory, and suppress all meteorological data after end date
	inifile=$(fgrep ${stn} to_exec.lst | awk '{for(i=1; i<=NF; i++) {if($i=="-c") {print $(i+1)}}}' | awk -F\/ '{print $NF}')
	inifile_add="./cfgfiles/stop_and_go_${stn}.ini"
	if (( ${n} > 0 )); then
		...
		echo "*::arg1::when    = ${d} - ${end_date}" >> ${inifile_add}
